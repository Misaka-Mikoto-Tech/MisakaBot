...
class MessageSegment(BaseMessageSegment["Message"]):
    ...
    def __str__(self) -> str:
        ...
        ignoreFields = ['file', 'fid', 'md5', 'name']
        params = " ,".join( # 加个空格，以便点击url时不会让后面的字段一起被识别
            f"{k}={escape(str(v))}" for k, v in self.data.items() if (v is not None) and (k not in ignoreFields)
        )
    ...
    def to_rich_text(self, truncate: Optional[int] = 7000000) -> str:
        ...
        ignoreFields = ['file', 'fid', 'md5', 'name'] # icqq 传过来太多参数，没必要显示
        params = " ,".join( # 加个空格，以便点击url时不会让后面的字段一起被识别
            f"{k}={rich_escape(str(v))}"
            for k, v in self.data.items()
            if (v is not None) and (k not in ignoreFields)
        )
