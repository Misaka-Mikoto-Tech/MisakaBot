...
:date: 2023.02.10
:brief: 抖音api
:async modify: Misaka-Mikoto-Tech at 2023-8-10
"""
import json
import random
import time
...
async def find_stream_url(room):
    json_info = await get_live_state_json(room.room_id)
...
async def get_live_state_json(room_id):
    api_url = get_api_url(room_id)
    async with AsyncClient() as client:
        req = await client.get(
            api_url, headers=get_request_headers(), timeout=10
        )
    res = req.text
...
async def get_danmu_ws_url(room_id, live_room_real_id, retry=0):
    if retry >= 5:
    ...
    if index_render_data == -1:
        logger.warning(f"cannot find RENDER_DATA when loading {room_id}")
        await cookie_utils.record_cookie_failed()
        return await get_danmu_ws_url(room_id, live_room_real_id, retry+1)

    info = req[index_render_data:]
    index = info.find("user_unique_id")
    if index == -1:
        logger.warning(f"cannot find user_unique_id when loading RENDER_DATA of {room_id}")
        return await get_danmu_ws_url(room_id, live_room_real_id, retry+1)
...
async def get_user_info(sec_user_id):
    ms_token = generate_random_str(132)
    headers = {
        'User-Agent': "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/109.0.0.0 Safari/537.36",
        'referer': 'https://www.douyin.com/',
        'Cookie': await cookie_utils.get_cookie_cache() + '; msToken=' + ms_token + '; odin_tt=324fb4ea4a89c0c05827e18a1ed9cf9bf8a17f7705fcc793fec935b637867e2a5a9b8168c885554d029919117a18ba69; passport_csrf_token=f61602fc63757ae0e4fd9d6bdcee4810;'
    }
    ...
    # Path(f"./dy_user_info.json").write_text(json.dumps(response, indent=2,ensure_ascii=False), encoding='utf8')
    params = response["param"]
    async with AsyncClient() as client:
        resp = await client.get(
            prefix + params, headers=headers
        )
...
async def is_going_on_live(room):
    room_json = await get_live_state_json(room.room_id)
